function MeshShader(){
		...
		this.fragmentShader = [
			...
			// "   gl_FragColor = vec4(color.rgb, color.a);",
			    ...
			    "float am1 = 0.5 + 0.5*sin( time*2.0 );",
			    "float am2 = 0.5 + 0.5*cos( time*2.0 );",
			    
			    "for( int i=0; i<20; i++ )",
			    "{",
			    "	float h  = dot( texture2D(texture, uv*0.99,               -100.0).xyz, vec3(1.0) );",
			    "	float h1 = dot( texture2D(texture, uv+vec2(e.x,0.0), -100.0).xyz, vec3(1.0) );",
			    "	float h2 = dot( texture2D(texture, uv+vec2(0.0,e.y), -100.0).xyz, vec3(1.0) );",
